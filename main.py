...
reserved = ["printa", "ler", "enquanto", "se", "senao", "e", "ou", "intero", "booleano", "texto", "verdade", "falso", "retorna"]
...
class Tokenizer:
    def __init__(self, _origin):
        ...
        self.position = 0
        self.actual = Token("INTERO", ' ')
        self.PreviousPosition = 0
        self.PreviousActual = Token("INTERO", ' ')
        self.selectNext()
    ...
    def selectNext(self):
        ...
        if self.position == len(self.origin):
            ...
            while self.position < len(self.origin) and self.origin[self.position].isnumeric():
                ...
                self.position += 1
            self.actual = Token('INTERO', int(num))
            ...
            while self.origin[self.position] != '"' and self.position < len(self.origin):
                ...
                self.position += 1
            self.position += 1
            self.actual = Token('TEXTO', str(string))
            ...
            if var not in reserved:
                ...
                if var == "intero" or var == "booleano" or var == "texto":
...
class Parser:
    ...
    def typeexpr():
        if Parser.tokens.actual.type == "TYPE":
            if Parser.tokens.actual.value == "INTERO":
                Parser.tokens.selectNext()
                return Type("INTERO", [])

            if Parser.tokens.actual.value == "BOOLEANO":
                Parser.tokens.selectNext()
                return Type("BOOLEANO", [])

            if Parser.tokens.actual.value == "TEXTO":
                Parser.tokens.selectNext()
                return Type("TEXTO", [])
    ...
    @staticmethod
    def factor():
        if Parser.tokens.actual.type == "INTERO":
        ...
        elif Parser.tokens.actual.type == "BOOLEANO":
            result = Parser.tokens.actual.value
            Parser.tokens.selectNext()
        ...
        elif Parser.tokens.actual.type == "TEXTO":
            # print(Parser.tokens.actual.value)
            result = Parser.tokens.actual.value
            Parser.tokens.selectNext()
            # print(Parser.tokens.actual.value)
            if Parser.tokens.actual.type == "TEXTO":
                raise ValueError('Erro')
        ...
        elif Parser.tokens.actual.type == "LER":
            Parser.tokens.selectNext()
            if Parser.tokens.actual.type == "OPEN":
                Parser.tokens.selectNext()
                if Parser.tokens.actual.type == "CLOSE":
    ...
    def command():
        ...
        elif Parser.tokens.actual.type == "RETORNA":
            Parser.tokens.selectNext()
        ...
        elif Parser.tokens.actual.type == "PRINTA":
            Parser.tokens.selectNext()
            if Parser.tokens.actual.type == "OPEN":
                Parser.tokens.selectNext()
                result = Parser.orexpr()
        ...
        elif Parser.tokens.actual.type == "ENQUANTO":
            Parser.tokens.selectNext()
            if Parser.tokens.actual.type == "OPEN":
                Parser.tokens.selectNext()
        ...
        elif Parser.tokens.actual.type == "SE":
            Parser.tokens.selectNext()
            if Parser.tokens.actual.type == "OPEN":
                Parser.tokens.selectNext()
                condition = Parser.orexpr()
        ...
        elif Parser.tokens.actual.type == "SENAO":
    ...
    def block():
        children = []
        end_semicolon = ["SEPARATOR", "IDENTIFIER", "PRINTA", "TYPE"]
        if Parser.tokens.actual.type == "OPEN_BLOCK":
            Parser.tokens.selectNext()
...
class If(Node):
    ...
    def Evaluate(self, symbol_table):
        ...
        if x[0] == True or (x[1] == "INTERO" and x[0] != 0):
...
class BinOp(Node):
    ...
    def Evaluate(self, symbol_table):
        ...
        if x[1] == "INTERO" and y[1] == "INTERO":
            if self.value == "+":
                return (x[0] + y[0], "INTERO")
            elif self.value == "-":
                return (x[0] - y[0], "INTERO")
            elif self.value == "*":
                return (x[0] * y[0], "INTERO")
        ...
        elif x[1] == "BOOLEANO" and y[1] == "BOOLEANO":
            if self.value == "&&":
                return (x[0] and y[0], "BOOLEANO")
            elif self.value == "||":
                return (x[0] or y[0], "BOOLEANO")
            elif self.value == "==":
                return (x[0] == y[0], "BOOLEANO")
        
        elif (x[1] == "BOOLEANO" and y[1] == "INTERO") or (x[1] == "INTERO" and y[1] == "BOOLEANO"):
            if self.value == "+":
                return (x[0] + y[0], "INTERO")
            elif self.value == "-":
                return (x[0] - y[0], "INTERO")
                ...
                return (x[0] or y[0], "INTERO")
        
        elif (x[1] == "TEXTO" and y[1] == "INTERO") or (x[1] == "INTERO" and y[1] == "TEXTO"):
            if self.value == "*":
                return (x[0] * y[0], "TEXTO")
            else:
                raise ValueError('ERRO: BinOp TEXTO + INTERO')
        
        elif x[1] == "TEXTO" and y[1] == "TEXTO":
            if self.value == "+":
                return (x[0] + y[0], "TEXTO")
            if self.value == "==":
                return (x[0] == y[0], "BOOLEANO")
            if self.value == "&&":
                return (x[0] and y[0], "TEXTO")
            elif self.value == "||":
                return (x[0] or y[0], "TEXTO")
        else:
...
class UnOp(Node):
    ...
    def Evaluate(self, symbol_table):
        if self.children[0].Evaluate(symbol_table)[1] == "INTERO":
            if self.value == "+":
                return (self.children[0].Evaluate(symbol_table)[0], "INTERO")
            elif self.value == "-":
                return (-self.children[0].Evaluate(symbol_table)[0], "INTERO")
            elif self.value == "!":
                # printERO(self.children[0].Evaluate(symbol_table))
                return (not self.children[0].Evaluate(symbol_table)[0], "BOOLEANO")
            else:
                raise ValueError('Erro')
        if self.children[0].Evaluate(symbol_table)[1] == "BOOLEANO":
            if self.value == "!":
                return (not self.children[0].Evaluate(symbol_table)[0], "BOOLEANO")
